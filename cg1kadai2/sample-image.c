...
GLenum	imageFormat;
int winWidth = 1000, winHeight = 500;
...
/*void init(void)
{
   ...
   glShadeModel(GL_FLAT);               //ayasii
   makeCheckImage();
   glPixelStorei(GL_UNPACK_ALIGNMENT, 1);//ayasii
   }*/
...
void display(void)
{
   ...
   glRasterPos2i((winWidth-imageWidth*zoomFactor)/2, (winHeight-imageHeight*zoomFactor)/2);
   glPixelZoom (zoomFactor, zoomFactor);

   glDrawPixels(imageWidth, imageHeight, imageFormat,
...
void motion(int x, int y)
{
   ...
   glPixelZoom (zoomFactor, zoomFactor);
   glCopyPixels (0, 0, imageWidth, imageHeight,imageFormat);
   glPixelZoom (1.0, 1.0);
   glFlush ();
}
